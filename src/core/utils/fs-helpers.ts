import { isOrdoDirectory } from "../guards/is-fs-entity"
import { OrdoFile, OrdoDirectory, Nullable } from "../types"
...
export const findParent = (
  ...
  for (const chunk of parentPathChunks) {
    const found = parent.children.find(
      child => child.path === `${parent.path}${chunk}/`
    )
...
export const sortOrdoDirectory = (directory: OrdoDirectory) => {
  directory.children = directory.children.sort((a, b) => {
    if (isOrdoDirectory(a)) {
      sortOrdoDirectory(a as any)
    }

    if (isOrdoDirectory(b)) {
      sortOrdoDirectory(b as any)
    }
